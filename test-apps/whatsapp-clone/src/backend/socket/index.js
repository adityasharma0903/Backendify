...
🚪 JOIN CONVERSATION REQUEST`);
        ...
        // Check if user is participant (handle both string and ObjectId)
        const isParticipant = conversation.participants.some(p => 
          p.toString() === socket.userId || p._id?.toString() === socket.userId
        );

        if (!isParticipant) {
          console.log(`  ❌ FAILED: User NOT a participant`);
          console.log(`     User ID: ${socket.userId}`);
          console.log(`     Participants: ${conversation.participants.map(p => p.toString ? p.toString() : p).join(', ')}`);
        ...
        console.log(`     - Participant IDs: ${conversation.participants.map(p => p.toString()).join(', ')}`);
