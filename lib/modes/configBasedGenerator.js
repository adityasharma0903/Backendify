...
export const requestLogger = morgan(':method :url :status :response-time ms');`;
  ...
  // Validation middleware (if enabled)
  if (config.enableValidation) {
    const validation = `export const validateRequest = (schema) => {
  return (req, res, next) => {
...
${config.database === 'mongodb' ? `    MongooseModule.forRoot(process.env.MONGODB_URI || 'mongodb://localhost:27017/backendify'),` : ''}
${config.database === 'postgresql' ? `    TypeOrmModule.forRoot({
      ...
      database: process.env.DB_NAME || 'backendify',
...
function createPackageJson(backendPath, config) {
  ...
  const packageJson = {
    name: 'backendify-backend',
    version: '1.0.0',
    description: 'Production-ready backend generated by Backendify',
...
function generateSchemaSQL(resources, dbType, apiAnalysis = { relationships: [] }) {
  ...
  let sql = `-- ============================================
-- Backendify ${dbType.toUpperCase()} Schema
...
function generateCRUDSQL(resources, dbType, apiAnalysis = { relationships: [] }) {
  let sql = `-- ============================================\n`;
  sql += `-- Backendify ${dbType.toUpperCase()} CRUD Operations\n`;
...
function generateJoinSQL(resources, dbType, apiAnalysis = { relationships: [] }) {
  let sql = `-- ============================================\n`;
  sql += `-- Backendify ${dbType.toUpperCase()} Relationships & JOINs\n`;
...
function generateSeedSQL(resources, dbType, apiAnalysis = { relationships: [] }) {
  let sql = `-- ============================================\n`;
  sql += `-- Backendify ${dbType.toUpperCase()} Sample Data\n`;
...
router.get('/', (req, res) => {
  res.json({ message: 'Welcome to Backendify API' });
});
...
export const createUser = async (req, res) => {
  ...
  // README
  const readme = `# Backendify Generated Backend

This is a production-ready backend generated by Backendify.
