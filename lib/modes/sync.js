...
mysql -u root -p ${process.env.DB_NAME || 'backendify'} < sql/01_schema.sql
\`\`\`
...
# Option 2: Command Line
psql -U postgres -d ${process.env.DB_NAME || 'backendify'} -a -f sql/01_schema.sql
\`\`\`
...
function generateSchemaSQL(resources, dbType) {
  ...
  let sql = `-- ============================================\n`;
  sql += `-- Backendify Auto-Generated SQL Schema\n`;
