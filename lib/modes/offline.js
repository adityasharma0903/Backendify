...
export async function offlineMode(projectPath) {
  try {
    ...
    const frontendContent = fs.readdirSync(projectPath).map(f => {
      try {
        return fs.readFileSync(path.join(projectPath, f), 'utf8');
      } catch {
        return '';
      }
    }).join('\n');
    ...
    for (const [resourceName, resource] of Object.entries(nonAuthResources)) {
      ...
      const fields = resource.fields.length > 0 
      ...
      fs.writeFileSync(
        path.join(backendPath, 'models', `${modelName}.js`),
        modelContent
      );

      // Generate Routes
      const routesContent = generateCrudRoutes(resourceName, fields, hasAuth);
      fs.writeFileSync(
        path.join(backendPath, 'routes', `${resourceName}.routes.js`),
        routesContent
      );
