...
function fixApiUrls(content, isVite = false) {
  ...
  // Pattern 2b: fetch(`${API_BASE}/api/...`) -> fetch(`${process.env.REACT_APP_API_URL}/api/...`)
  content = content.replace(
    /fetch\s*\(\s*`\$\{[^}]+\}(\/api\/[^`]+)`/g,
    `fetch(\`\${${API_URL}}$1\``
  );
  ...
  // Pattern 3b: const url = `${API_BASE}/api/...` -> const url = `${process.env.REACT_APP_API_URL}/api/...`
  content = content.replace(
    /(const|let|var)\s+(\w+)\s*=\s*`\$\{[^}]+\}(\/api\/[^`]+)`/g,
    `$1 $2 = \`\${${API_URL}}$3\``
  );
  ...
  // Pattern 4b: axios.get(`${API_BASE}/api/...`) -> axios.get(`${process.env.REACT_APP_API_URL}/api/...`)
  content = content.replace(
    /\b(axios|api|client|http|request)\.(get|post|put|delete|patch)\s*\(\s*`\$\{[^}]+\}(\/api\/[^`]+)`/g,
    `$1.$2(\`\${${API_URL}}$3\``
  );
