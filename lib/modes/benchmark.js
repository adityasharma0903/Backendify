/**
 * Backendify Benchmark - Scalability & Performance Testing
...
export async function runBenchmark(projectPath, options) {
  console.log(chalk.cyan('\n⚡ Backendify Benchmark - Scalability Testing\n'));
  ...
  if (routes.length === 0) {
    console.log(chalk.yellow('\n⚠️  No routes detected. Make sure your backend has API endpoints.\n'));
    return;
  }
  ...
  if (benchmarkRoutes.length === 0) {
    console.log(chalk.yellow('⚠️  No benchmark-safe routes found (all routes require dynamic params).'));
    return;
  }
