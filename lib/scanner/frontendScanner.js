...
function runAstDetection(projectPath) {
  ...
  for (const file of files) {
    ...
    for (const ast of parsedTargets) {
...
export function buildHybridIR(projectPath) {
  const { apiCalls, forms } = runAstDetection(projectPath);
