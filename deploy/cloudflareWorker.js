...
function sanitizeProjectName(rawValue, fallback = 'backendify-api') {
...
function ensurePagesBackendScaffold(backendPath, projectName) {
  const deployDir = path.join(backendPath, '.backendify-cloudflare-pages');
...
export async function deployToCloudflareWorker(backendPath, options = {}) {
  let projectName = '';
  let deployDir = path.join(backendPath, '.backendify-cloudflare-pages');

  const preflight = async () => {
    projectName = sanitizeProjectName(options.projectName || path.basename(backendPath), 'backendify-api');
